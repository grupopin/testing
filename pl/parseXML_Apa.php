...
$xmlPath="/home/hwarch/domains/hw-archive.com/faust_import";

$fileName=$xmlPath."/110509/APA_all.xml";
$xml=new XMLReader();
$xml->open($fileName);
$now=date("Y-m-d_His");
$fpOut=fopen("kbase/xml_apa_$now.pl", 'w+');
//print $fpOut;
//exit();
...
function strToCamel( $str, $firstUpper=true){
...
function outTerm($termName, $termBody){
  global $sameTermsTogether, $kbArr, $fpOut, $termPrefix;
  $termStr2=$termPrefix.$termName.'('.$termBody.').'.PARSE_NewLine;
  if($sameTermsTogether){
...
function coordinator( $str, $curId){
  ...
  if($new){
    $coordPersStr="$coordId,'$coordinator'";
    outTerm('CoordinatorPerson', $coordPersStr);
  }
  $coordStr="$curId,$coordId";
  outTerm('Coordinator', $coordStr);
...
function litMonographs( $str, $curId){
  ...
  outTerm("LiteratureMonographs", $monoStr);
...
function litExhibCatalogs( $str, $curId){
  ...
  if($new){
    $exhibCatalogStr="$exhibCatalogId,'$exhibCatalog'";
    outTerm('ExhibCatalog', $exhibCatalogStr);
  ...
  outTerm("LiteratureExhibitionCatalogs", $monoStr);
...
function groupExhibitions( $str, $curId, $index){
  ...
  if($new){
    $galStr="$galId,'$galleryName'";
    outTerm("GalleryName", $galStr);
  }

  $galTermStr="$curId,$galId,'$galleryOrg','$galleryCity'";
  outTerm("Gallery", $galTermStr);
  ...
  $oneManExhibTerm="$curId,$galId,{$parserData->getCurrentExbGroupId()},$year1,$year2,'$portfolio'";
  outTerm("GroupExhibition", $oneManExhibTerm);
...
function oneManExhibitions( $str, $curId, $index){
  ...
  if((($pos==$len)&&$foundStr)||$foundTravelling){
    ...
    $galGroupTerm="$grId,'Travelling','{$match[1]}',$year1,$year2";
    outTerm("GroupGallery", $galGroupTerm);
    ...
    $oneManExhibTerm="$curId,$galId,{$parserData->getCurrentExbGroupId()},$year1,$year2,'$portfolio','$exhibAB'";
    outTerm("OneManExhibition", $oneManExhibTerm);
