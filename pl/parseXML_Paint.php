...
//files will be parsed one by one, same primary key will be maintained for all files
//One PL file will be generated
$now=date("Y-m-d_His");
$outFileName="xml_paint_{$now}.pl";
$outDir="kbase";

$files[]=$xmlPath."/JW_all_090511.xml";//junior works
$files[]=$xmlPath."/Paint_all_090511.xml";//junior works
...
$count=array();
$fpOut=fopen($outDir.'/'.$outFileName,'w+');
...
function groupExhibitions($str,$curId,$index){
	...
	switch($cnt){
		...
		case 5:
			$galleryName=trim($tmpArr[0]);
			$galleryOrg=trim($tmpArr[1]).','.trim($tmpArr[2]);
			$galleryCity=trim($tmpArr[3]);
			preg_match($yearRegex,trim($tmpArr[4]),$yrMatch);
			$year1=$yrMatch['year1'];
			$year2=$yrMatch['year2'];
			if ($yrMatch['portfolio']){
				$portfolio='portfolio';
